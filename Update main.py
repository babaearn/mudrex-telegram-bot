...
HTTP_TIMEOUT = 5
EXCHANGEINFO_TTL = 30 * 60  # 30 minutes
...
def _http_get(path: str, params: dict) -> Optional[requests.Response]:
    """Robust HTTP GET with automatic failover across Binance endpoints."""
    last_exc = None
    ...
    log.error(f"HTTP GET failed for {path}: {last_exc}")
    return None

# -----------------------
# ExchangeInfo Cache & Symbol Helpers
# -----------------------
...
def _normalize_command(text: str) -> Tuple[str, Optional[str]]:
    ...
    Examples: '/btcUsdt 15m@Bot' -> ('BTCUSDT', '15m')
    """
    s = text.strip()
    if s.startswith("/"):
        s = s[1:]
...
def _resolve_symbol_or_fallback(symbol: str) -> Tuple[Optional[str], Optional[str]]:
    """
    Resolve symbol with smart fallback to different quote currencies.
    Returns (resolved_symbol, note).
    """
    # Check if symbol exists as-is
    if _symbol_exists(symbol):
        return symbol, None

    # If symbol has a quote currency, try alternates
...
def fetch_klines(symbol: str, interval: str, limit: int = 100) -> Optional[List[List]]:
    ...
    r = _http_get("/api/v3/klines", params={
        "symbol": symbol,
        "interval": interval,
        "limit": limit
    })
...
class TechnicalAnalysis:
    """Technical analysis calculations for price data."""
...
def build_update(symbol_in: str, interval_in: Optional[str]) -> str:
    ...
    if not resolved:
        return (
            f"⚠️ Symbol `{symbol}` not found on Binance.\n"
...
def main():
    ...
    if not TELEGRAM_TOKEN:
        ...
        log.error("Set it in environment variables or .env file")
        return
